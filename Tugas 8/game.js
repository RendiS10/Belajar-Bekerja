document.addEventListener("DOMContentLoaded", function () {
  ...
  // Fungsi untuk menampilkan riwayat ke web
  function renderHistory(filter = "Semua") {
    ...
    if (filtered.length === 0) {
      historyArea.innerHTML = `<div class="text-center text-muted">Belum ada riwayat permainan.</div>`;
      return;
    }
    let html = `<table class="table table-bordered table-sm mt-2"><thead><tr><th>#</th><th>Pilihan Anda</th><th>Pilihan Komputer</th><th>Hasil</th></tr></thead><tbody>`;
    filtered.forEach((item, idx) => {
      html += `<tr><td>${idx + 1}</td><td>${item.user}</td><td>${
        item.computer
