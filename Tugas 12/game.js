document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    // Tampilkan animasi/teks komputer memilih
    computerPick.textContent = "Komputer sedang memilih...";
    ...
    userPick.textContent = `${user} (Kamu)`;
    try {
      // Simulasi komputer memilih (1.5 detik)
      ...
      setTimeout(() => {
        const lastRow = document.querySelector(
          "#historyArea tr.history-row-new"
        );
        if (lastRow) lastRow.classList.remove("history-row-new");
      }, 1200);
    } catch (err) {
      computerPick.textContent = "Terjadi kesalahan. Silakan coba lagi.";
      gameResult.textContent = "Error: " + (err?.message || err);
      gameResult.className = "alert mt-3 alert-danger";
      playAgainArea.style.display = "block";
    }
  });
  ...
  document.addEventListener("click", function (e) {
    if (e.target && e.target.id === "playAgainBtn") {
      // Tampilkan konfirmasi main lagi
      if (confirm("Main lagi?")) {
        ...
        // Jika tidak, tampilkan pesan terima kasih dan disable form
        resultArea.style.display = "block";
        playAgainArea.style.display = "none";
        userPick.textContent = "";
        computerPick.textContent = "";
        gameResult.textContent = "Terima kasih sudah bermain!";
        gameResult.className = "alert mt-3 alert-info";
        form.querySelector('button[type="submit"]').disabled = true;
        userChoice.disabled = true;
      }
    }
